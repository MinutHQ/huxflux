...
- `AgentShortcuts` — headless component that wires the agent-level keyboard shortcuts (⌘⇧D mark done, ⌘⇧⌫ archive); mounted once by `routes/_app.tsx` inside the workspace provider
...
- Agent-level shortcuts also arrive as window events: `huxflux:agent-done` (⌘⇧D) sets the open agent's status to `done`, and `huxflux:archive-agent` (⌘⇧⌫) opens a confirm toast whose action runs the same `deleteAgent` flow as the row context menu. `useAgentShortcuts` owns both subscriptions and resolves the target from the route (`/agent/$agentId`) rather than the hovered row or the active chat tab, which may be a thread child. It is hosted by the `AgentShortcuts` component rather than `AgentList` because the sidebar unmounts on the PR / Refine tabs and renders twice while collapsed (panel plus floating overlay), which would double-fire every chord.
- Agent-creation shortcuts arrive as window events from the root route's global key listener: `huxflux:new-agent` (⌘N, opens the repo picker) and `huxflux:new-agent-for-repo` (⌘1-9, `detail.index` picks the repo in `useRepos` order and creates a worktree agent immediately). `useNewAgentShortcuts` owns both subscriptions; `buildNewAgentArgs` in `agentListUtils.ts` is the single place the title/branch/direct triple is derived so the picker and the shortcut spawn identical agents. In a regular browser ⌘1-9 is reserved for tab switching and never reaches the page; the shortcut is effectively desktop (Tauri) only.
