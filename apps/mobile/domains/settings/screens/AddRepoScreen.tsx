...
// `c.accent` is not defined on the mobile theme — preserved as in the agents
// domain until the theme is properly typed.
const accent = (c as Record<string, string>).accent

function DiscoverList({
  loading, filtered, onSelect, search, setSearch,
}: {
  loading: boolean
  filtered: FoundRepo[]
  onSelect: (r: FoundRepo) => void
  search: string
...
function ConfirmDetails({
  selected, name, setName, branchFrom, setBranchFrom, loadingBranch,
  onChangeRepo, onSubmit, submitting, canSubmit,
}: {
  selected: FoundRepo
  name: string
  ...
  return (
    <View style={{ flex: 1, padding: 16, gap: 16 }}>
      ...
      <TouchableOpacity
        onPress={onSubmit}
        disabled={!canSubmit}
        style={{ backgroundColor: canSubmit ? accent : c.secondary, borderRadius: 10, paddingVertical: 14, alignItems: "center" }}
      >
        {submitting
...
export function AddRepoScreen() {
  ...
  useEffect(() => {
    api.repos.findRepos().then((r) => setDiscovered(r)).catch(() => {}).finally(() => setLoadingDiscovered(false))
  }, [])
  ...
  async function handleSelect(repo: FoundRepo) {
    ...
    try {
      // fire-and-forget; intentional: one-off branch lookup chained off a list selection
      // eslint-disable-next-line no-restricted-syntax
      const res = await api.repos.defaultBranch(repo.path)
      setBranchFrom(res.branch)
    } catch { /* keep default */ }
    finally { setLoadingBranch(false) }
  }

  const createRepo = useHuxfluxMutation<unknown, { name: string; path: string; branchFrom: string; remote: string }>({
    mutationFn: (body) => api.repos.create(body),
    invalidate: () => queryKeys.repos.all,
    onSuccess: () => router.back(),
    onError: (e) => modal.showAlert("Error", (e as Error).message ?? "Failed to add repo"),
  })
  const submitting = createRepo.isPending

  function handleSubmit() {
    const repoPath = selected?.path ?? path.trim()
    const repoName = name.trim() || (repoPath.split("/").pop() ?? "repo")
    if (!repoPath || submitting) return
    createRepo.mutate({ name: repoName, path: repoPath, branchFrom, remote: "origin" })
  }
    ...
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      {!selected ? (
        ...
        <ConfirmDetails
          ...
          onChangeRepo={() => { setSelected(null); setName(""); setPath(""); setBranchFrom("origin/main") }}
