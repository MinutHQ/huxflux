...
- `expo-image-picker`, `expo-file-system` — chat image attachments; `expo-file-system/legacy` for the Android Storage Access Framework used by the file download
...
- **File download is Android-only.** Long-pressing a file in the files screen (either the changed-file rows or the file tree) opens an action sheet with a save entry, handled by `hooks/useFileDownload.ts`. Saving writes into a folder the user picks through Android's Storage Access Framework, which lives in `expo-file-system/legacy` — the new `expo-file-system` API has no equivalent. iOS has no SAF, so `canSaveToPhone` is false there and the long-press does nothing. The picked folder is remembered in `prefs.getDownloadDir()`; a write failure clears it and asks again once, covering a deleted folder or a revoked grant.
- **SAF renames files whose extension does not match their MIME type.** `createFileAsync` appends the extension Android maps to the MIME type it was given, so `App.tsx` created as `text/plain` lands as `App.tsx.txt`. `useFileDownload` declares the real type for the extensions Android knows (`.json`, `.md`, `.png`, …) so those names survive; source files fall back to `text/plain` and may pick up a `.txt` suffix. Android also de-duplicates display names, so the success alert reports the name parsed back out of the created document URI, not the requested one.
- **Downloads are text-only.** The save path reuses the plain-text `files/content` and `files/diff` endpoints, which is all the server exposes. Binary files in the tree (images, archives) will be written as mangled UTF-8, the same way the existing file viewer already renders them.
