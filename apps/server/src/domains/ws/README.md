...
- `handler.ts` — connection registration, per-agent subscription map, `broadcast()` / `emit()` primitives. Parses client frames against `clientEventSchema` and drops malformed envelopes.
...
- `handler.ts` — `registerSocket`, `broadcast`, `emit`.
