...
CLI provider abstractions: Claude (streaming + interactive), Claude Agent SDK (in-process `@anthropic-ai/claude-agent-sdk`), Codex (OpenAI), Gemini (Google), and Pi (`@earendil-works/pi-coding-agent`). Each provider exposes a normalized streaming interface that the agents runner consumes. Provider availability detection (which CLIs are installed) lives in `domains/settings` — only the provider runtimes themselves live here.

## Owns

- The provider registry (`getProvider` / `getAvailableProviders` / `getInstalledProviders`) and the provider adapters: `claudeProvider`, `agentSdkProvider`, `claudeInteractiveProvider`, `codexProvider`, `geminiProvider`, `piProvider`.
- The `ProviderAdapter` interface: `resolveBinary`, `isAvailable`, `buildSpawnArgs`, `parseStreamLine`, `resolveModel`, `getModels`, optional `runTurn` (in-process turn as an async iterator of normalized events; when present the runner never calls `buildSpawnArgs`), `installHooks`, `sessionFilePath`, `continueProbePath`.
...
- `providers.types.ts` — `ProviderId`, `ProviderAdapter`, `NormalizedStreamEvent`, `SpawnOptions`, `SpawnResult`, `RunTurnContext`, `PermissionRequest`, `PermissionDecision`, `ProviderCapabilities`.
...
- **agent-sdk runs in-process.** `agentSdkProvider.runTurn` calls the SDK's `query()` with the runner's `SpawnOptions` (`systemPrompt: { preset: "claude_code", append }`, `bypassPermissions` unless plan mode, `resume` / `continue`, `effort`, `settingSources: ["user","project","local"]` so CLAUDE.md and user settings load like the CLI) and maps each `SDKMessage` through the pure `service/agentSdkEvents.ts` mapper. `buildSpawnArgs` throws because the runner never uses it for this provider; `parseStreamLine` exists for interface parity only. The prompt is a streaming input (`buildPromptStream`): the initial message followed by `ctx.userMessages`, so mid-run injection works and the SDK session stays open until the runner closes the input after `done`. `canUseTool` forwards to `ctx.requestPermission`, which is how AskUserQuestion reaches the UI (the runner parks it and answers with `updatedInput`; everything else is denied). `forwardSubagentText: true` and `allowedTools` always including `AskUserQuestion` mirror the CLI adapter. The SDK still spawns its own bundled Claude Code binary, so it ignores `CLAUDE_BIN` and may run a different CLI version than the `claude` adapter, but it writes the same `~/.claude/projects` transcripts, so `sessionFilePath` / `continueProbePath` are shared with `claude`. Not covered: the firejail sandbox wrapper (spawn path only). The SDK logs a `CLAUDE_SDK_CAN_USE_TOOL_SHADOWED` warning on every turn because `bypassPermissions` skips `canUseTool` for ordinary tools; AskUserQuestion still goes through it (verified end to end), and the callback is what makes the SDK expose the tool at all, so the warning is expected noise.
...
7. If the provider is a library rather than a CLI, implement `runTurn(opts, ctx)` instead: yield `NormalizedStreamEvent`s, honour `ctx.signal` (stop promptly on abort), send diagnostics to `ctx.onStderr`, pass `ctx.env` to any subprocess you start, consume `ctx.userMessages` for mid-run injection, and route permission prompts through `ctx.requestPermission`. Keep the SDK-to-event mapping in a separate pure module with its own test (see `agentSdkEvents.ts`).
