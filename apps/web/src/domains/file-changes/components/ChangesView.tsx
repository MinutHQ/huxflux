...
export const ChangesView = React.memo(function ChangesView({
  ...
  type AllDiff = NonNullable<typeof allDiffs>[number]
  const diffsByPath = useMemo(() => {
    if (!allDiffs) return new Map<string, AllDiff>()
  ...
  useEffect(() => {
    ...
    if (el) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional: uncollapse the target file before scrolling to it
