...
- The agent-facing `<huxflux:agents.*>` tag handlers (title, branch, delegate, spawn, fork) in `service/runnerTags.ts`, the per-turn tag-instruction builders in `service/tagInstructions.ts`, and chat-run option assembly in `service/chatRun.ts`. The top-level facades re-export them for cross-domain consumers.
...
- `agents.ws.ts` — typed event builder `agentsWs` and the `AgentsServerEvent` union (includes `background:state`, emitted by the runner when the CLI's background work changes, and `tasks:state`, emitted after a TaskCreate / TaskUpdate result with the re-read task list). Consumed by `src/domains/ws/events.ts` to compose the central `ServerEvent`. Also consumed by `src/domains/agent-runner/`, which broadcasts agents events from inside the runner.
...
- `runnerTags.ts` / `tagInstructions.ts` / `chatRun.ts` — re-export facades over `service/runnerTags.js`, `service/tagInstructions.js`, `service/chatRun.js`. The service/ folder holds the implementations (private); cross-domain call sites (chat, automations, tasks, refine) import the facades so the runner stays domain-agnostic.
...
- `/clear` is a command, not a prompt. `service/clearConversation.ts` deletes the agent's messages and tool calls, nulls `sessionId`, drops the in-memory queue, removes the CLI's on-disk task list for the agent (`clearTaskList`, emitting an empty `tasks:state`), and emits `messages:cleared`. The next turn then bootstraps as a first turn (no `--resume`, empty `buildConversationContext`). Reached via `POST /api/agents/:id/clear` and by `POST /messages` intercepting a body whose trimmed content is exactly `/clear`. Refused with 409 while a turn is running.
...
- Renaming and the prompt cache: `service/tagInstructions.ts` never embeds the agent's title, and `buildNamingTurnContext` returns a per-turn note (passed as `turnContext` to the runner) only while the title or branch suffix still matches the placeholder pattern. Once both are real the note disappears, so renamed agents stop re-emitting the naming tags and the system prompt stays cache-stable.
- Cross-repo thread agents (`agents.spawn`): gated by the `threadsEnabled` setting, which now defaults to `true` (new profiles get spawn out of the box; the web Experimental toggle can still opt out, and `settings.json` on existing installs keeps whatever value was saved). When disabled, the spawn tag is not advertised in the system prompt and emitting one yields a "disabled" follow-up instead of a silent strip.
- Thread-agent spawn (`service/runnerTags.ts`, `agentSpawnHandler`) reports every failure back to the parent agent as a `system` follow-up (unknown repo, folder repo, missing repo path, worktree-creation failure, setup-script failure) — never a silent no-op. On setup-script failure the half-spawned agent row and worktree are rolled back.
- Thread-agent branch AND worktree location derive from the same sanitized task slug with an id suffix (`thread-<slug>-<id6>`): the suffix collision-proofs same-text spawns, and keeping `location` equal to the branch-minus-prefix keeps the pre-spawn `reconcileWorktreeLocation` a no-op — a mismatched location/branch pair gets the worktree moved out from under the running turn.
- The spawned agent inherits `model`/`provider` from the parent (falling back to the settings defaults), gets the create-route treatment (streamed setup script, live file watcher, default `t1` terminal tab), and its first message is seeded in-process via `enqueue` + `drainQueue` — no fire-and-forget HTTP fetch, so the task can never be lost and the first turn starts immediately.
- Thread children are retired (`agents.update.routes.ts`, `retireThreadChildren`) when the parent reaches a terminal status.
