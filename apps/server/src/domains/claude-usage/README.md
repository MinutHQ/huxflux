...
- The endpoint never throws. A missing token, a 401/403 (bad/revoked token), or any failure with no prior reading resolves to `{ connected: false, reason, error }` so the sidebar hides the provider when no usage data is available. Transient failures (timeout, 429, 5xx, network errors) return the last good reading while a token is still present, so a single flaky poll doesn't blank the bars. The cache is dropped on sign-out (no token) and on auth failure.
