...
- `MessageBubble` renders the last tool call's `precedingText` as the message body when a finished turn has no `content` (turn ended on an AskUserQuestion, a stop, or a trailing tool call). Otherwise the model's last words sit inside the collapsed tool-call accordion and the reply looks missing.
