...
export interface JiraIssue {
  ...
  fields: {
    ...
    [key: string]: any  // custom fields (e.g. sprint)
  }
}
...
export async function searchIssues(jql: string, maxResults = 50): Promise<JiraIssue[]> {
  ...
  if (sprintFieldId) {
    for (const issue of data.issues) {
      const raw = (issue.fields as any)[sprintFieldId]
      if (!issue.fields.sprint && Array.isArray(raw)) {
        const active = raw.find((s: any) => s.state === "active")
        issue.fields.sprint = active ?? raw[raw.length - 1] ?? null
      } else if (!issue.fields.sprint && raw && typeof raw === "object") {
        issue.fields.sprint = raw
      }
    }
  }
