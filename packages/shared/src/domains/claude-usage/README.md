...
- The `ClaudeUsage` shape returned by `/api/claude/usage` (connected flag, the 5-hour `session` window, the 7-day `weekly` window, the `spend` extra-usage total with its hour/day/week deltas, and an optional error string)
...
- `claudeUsageSchema`, `claudeUsageWindowSchema`, `claudeUsageSpendSchema`, `claudeUsageSpendDeltasSchema` — Zod schemas for `/api/claude/usage`
- `ClaudeUsage`, `ClaudeUsageWindow`, `ClaudeUsageSpend`, `ClaudeUsageSpendDeltas` — inferred types
